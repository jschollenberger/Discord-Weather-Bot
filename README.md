...
- **Morning briefing** *(optional)* — a compact, silent once-daily post with the day's forecast shape, high/low, tide times, sunrise/sunset, AQI, and alert state; it deliberately skips "right now" conditions (the pinned message already covers those). Enable with `briefing_enabled`
...
| `briefing_enabled` | false | Post a compact, silent daily **morning briefing** (forecast shape, high/low, tides, sunrise/sunset, AQI, alert state) to the conditions channel — no current-conditions duplication, no buttons |
